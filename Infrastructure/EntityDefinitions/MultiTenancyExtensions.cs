...
internal static class MultiTenancyExtensions
{
    ...
    private static void SetTenantOwnedFilter<T>(ModelBuilder modelBuilder)
        where T : class, ITenantOwned
    {
        modelBuilder.Entity<T>().HasQueryFilter(e => e.TenantId == ((ApplicationDbContext)null!).CurrentTenantId);
    }
}